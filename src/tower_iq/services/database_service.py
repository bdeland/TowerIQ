...
class DatabaseService:
    ...
    def bulk_update_runs_end(self, runs_end_data: List[Dict[str, Any]]) -> None:
        ...
        for run_data in runs_end_data:
            ...
            run_updates.append((
                safe_int(run_data.get('end_time', 0)) if run_data.get('end_time') is not None else None,
                safe_int(run_data.get('final_wave', 0)) if run_data.get('final_wave') is not None else None,
                safe_int(round_coins),
                safe_int(duration_realtime),
                safe_int(run_data.get('duration_gametime')),
                CPH,  # Already converted to integer
                safe_int(run_data.get('round_cells')),
                safe_int(run_data.get('round_gems')),
                safe_int(run_data.get('round_cash')),
                run_id_blob
            ))
    ...
    def backup_database(self, backup_path: Optional[str] = None) -> bool:
        ...
        try:
            ...
            try:
                ...
                candidates = sorted(
                    list(backup_dir_final.glob(f"{prefix}*.sqlite*")),
