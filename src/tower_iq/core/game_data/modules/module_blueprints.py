...
ALL_MODULE_BLUEPRINTS = [
    ...
    ModuleBlueprint(
        ...
        possible_substats=list(substats.ALL_SUBSTATS if s.applies_to == ModuleType.CANNON)
    ),
    ModuleBlueprint(
        name="Matter Cannon",
        module_type=ModuleType.CANNON,
    ...
    ModuleBlueprint(
        ...
        possible_substats=list(substats.ALL_SUBSTATS if s.applies_to == ModuleType.ARMOR)
    ),
    ModuleBlueprint(
        name="Matter Barrier",
        module_type=ModuleType.ARMOR,
    ...
    ModuleBlueprint(
        ...
        possible_substats=list(substats.ALL_SUBSTATS if s.applies_to == ModuleType.CORE)
    ),
    ModuleBlueprint(
        name="Matter Chip",
        module_type=ModuleType.CORE,
    ...
    ModuleBlueprint(
        ...
        possible_substats=list(substats.ALL_SUBSTATS if s.applies_to == ModuleType.GENERATOR)
    ),
    ModuleBlueprint(
...
BLUEPRINTS_BY_TYPE = {
    ModuleType.CANNON: list(ALL_MODULE_BLUEPRINTS if bp.module_type == ModuleType.CANNON),
    ModuleType.ARMOR: list(ALL_MODULE_BLUEPRINTS if bp.module_type == ModuleType.ARMOR),
    ModuleType.CORE: list(ALL_MODULE_BLUEPRINTS if bp.module_type == ModuleType.CORE),
    ModuleType.GENERATOR: list(ALL_MODULE_BLUEPRINTS if bp.module_type == ModuleType.GENERATOR),
}

# Organized by pull rarity
BLUEPRINTS_BY_PULL_RARITY = {
    Rarity.COMMON: list(ALL_MODULE_BLUEPRINTS if bp.can_be_pulled_at(Rarity.COMMON)),
    Rarity.RARE: list(ALL_MODULE_BLUEPRINTS if bp.can_be_pulled_at(Rarity.RARE)),
    Rarity.EPIC: list(ALL_MODULE_BLUEPRINTS if bp.can_be_pulled_at(Rarity.EPIC)),
}
...
def get_natural_epic_blueprints(module_type: ModuleType) -> List[ModuleBlueprint]:
    ...
    return list(ALL_MODULE_BLUEPRINTS if bp.is_natural_epic and bp.module_type == module_type)
...
def get_blueprint_statistics() -> Dict[str, int]:
    ...
    stats = {
        ...
        "common_modules": len(list(ALL_MODULE_BLUEPRINTS if bp.natural_rarity == Rarity.COMMON)),
        "rare_modules": len(list(ALL_MODULE_BLUEPRINTS if bp.natural_rarity == Rarity.RARE)),
        "natural_epics": len(list(ALL_MODULE_BLUEPRINTS if bp.is_natural_epic)),
    }
...
def validate_blueprints():
    ...
    duplicates = list(set(names) if names.count(name) > 1)
    if duplicates:
