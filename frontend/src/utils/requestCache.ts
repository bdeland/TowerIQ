...
export class RequestCache {
  ...
  constructor(config: CacheConfig = {}) {
    ...
    this.cleanupIntervalId = setInterval(() => this.cleanup(), 30000); // Cleanup every 30 seconds
  }
  ...
  invalidatePattern(pattern: string | RegExp): void {
    ...
    keysToDelete.forEach((key) => {
      ...
      if (this.config.enableLogging) {
        console.log(
          `RequestCache: Invalidated cache for ${key} (pattern match)`
        );
      }
    });
  }
  ...
  dispose(): void {
    ...
    if (this.config.enableLogging) {
      console.log("RequestCache: Disposed and cleaned up resources");
    }
  }
  ...
  private cleanup(): void {
    ...
    if (this.config.enableLogging && expiredKeys.length > 0) {
      console.log(
        `RequestCache: Cleaned up ${expiredKeys.length} expired entries (${before} -> ${this.cache.size})`
      );
    }
  }
