...
import { CacheKeys, defaultRequestCache } from "../utils/requestCache";
...
let globalStatusListeners: Set<(status: BackendStatus | null) => void> =
  new Set();

// Disable polling only in development mode for debugging purposes
const DISABLE_POLLING = import.meta.env.DEV ? false : false;
...
export const useBackend = () => {
      ...
      const result = await defaultRequestCache.get(
        ...
        async () => {
          ...
          const timeoutPromise = new Promise<never>((_, reject) => {
            setTimeout(
              () => reject(new Error("Device scanning timed out")),
              45000
            ); // 45 second timeout
          });
    ...
    try {
      ...
      const result = await defaultRequestCache.get(
        ...
        async () => {
          ...
          const timeoutPromise = new Promise<never>((_, reject) => {
            setTimeout(
              () => reject(new Error("Device refresh timed out")),
              45000
            ); // 45 second timeout
          });

          const refreshPromise = invoke<{ devices: Device[] }>(
            "refresh_devices"
          );
  ...
  const getHookScripts = async (): Promise<HookScript[]> => {
    try {
      ...
      const result = await defaultRequestCache.get(
        ...
        async () => {
          const result = await invoke<{ scripts: HookScript[] }>(
            "get_hook_scripts"
          );
    ...
    try {
      ...
      const result = await defaultRequestCache.get(
        ...
        async () => {
          ...
          const result = await invoke<{ frida_status: FridaStatus }>(
            "get_frida_status",
            { deviceId }
          );
  ...
  const activateHook = async (
    ...
    try {
      setError(null);
      const result = await invoke("activate_hook", {
        deviceId,
        processInfo,
        scriptId,
      });
