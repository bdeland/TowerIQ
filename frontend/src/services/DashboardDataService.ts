...
export class DashboardDataService {
  ...
  async fetchPanelData(
    ...
    if (this.pendingRequests.has(cacheKey)) {
      if (import.meta.env.DEV) {
        console.log(
          `DashboardDataService: Deduplicating request for panel ${panelId}`
        );
      }
  ...
  async fetchSinglePanel(
    ...
    try {
      ...
      console.error(
        `DashboardDataService: Error fetching single panel ${panel.id}:`,
        error
      );
      return {
        data: [],
  ...
  async fetchAllPanels(
    ...
    if (import.meta.env.DEV) {
      console.log(
        `DashboardDataService: Fetching data sequentially for ${panelsWithQueries.length} panels`
      );
    }
    ...
    for (let i = 0; i < panelsWithQueries.length; i++) {
      ...
      try {
        if (import.meta.env.DEV) {
          console.log(
            `DashboardDataService: Fetching panel ${i + 1}/${
              panelsWithQueries.length
            }: ${panel.id}`
          );
        }

        const result = await this.fetchPanelData(
          panel.id,
          panel.query!,
          variables
        );
        ...
        console.error(
          `DashboardDataService: Error fetching panel ${panel.id}:`,
          error
        );
    ...
    if (import.meta.env.DEV) {
      ...
      console.log(
        `DashboardDataService: Completed loading ${results.size} panels (${successCount} success, ${errorCount} errors)`
      );
    }
  ...
  private async executeQueryWithRetry(
    ...
    try {
      if (import.meta.env.DEV) {
        console.log(
          `DashboardDataService: Executing query for panel ${panelId}`
        );
      }
      ...
      if (query.includes("${")) {
        ...
        console.warn(
          `DashboardDataService: ${errorMessage} for panel ${panelId}`
        );
        return {
          data: [],
          loading: false,
      ...
      if (!response.ok) {
        ...
        if (response.status >= 500) {
          console.error(
            `DashboardDataService: Server error for panel ${panelId}:`,
            errorMessage
          );
        } else if (response.status >= 400) {
          console.error(
            `DashboardDataService: Client error for panel ${panelId}:`,
            errorMessage
          );
        }
      ...
      if (import.meta.env.DEV) {
        console.log(
          `DashboardDataService: Query successful for panel ${panelId}, ${
      ...
      if (import.meta.env.DEV) {
        console.error(
          `DashboardDataService: Query execution failed for panel ${panelId}:`,
          {
            error: errorMessage,
        ...
        console.error(
          `DashboardDataService: Query failed for panel ${panelId}:`,
          errorMessage
        );
      }
