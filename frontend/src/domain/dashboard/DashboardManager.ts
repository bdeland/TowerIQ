...
function mapToEChartsOptions(config: unknown): Partial<EChartsOption> | undefined {
  if (config && typeof config === 'object') {
    return config as Partial<EChartsOption>;
  }
  return undefined;
}

function mapToTableConfig(config: unknown): TableConfig | undefined {
  if (!config || typeof config !== 'object') {
    return undefined;
  }
  ...
  if (Array.isArray(record.columns)) {
    tableConfig.columns = record.columns.map((column) => String(column));
    hasValue = true;
  }

  if (typeof record.sortable === 'boolean') {
    tableConfig.sortable = record.sortable;
    hasValue = true;
  }

  if (typeof record.filterable === 'boolean') {
    tableConfig.filterable = record.filterable;
    hasValue = true;
  }

  if (typeof record.pagination === 'boolean') {
    tableConfig.pagination = record.pagination;
    hasValue = true;
  }

  const pageSize = typeof record.pageSize === 'number' ? record.pageSize :
    (typeof record.page_size === 'number' ? record.page_size : undefined);
  if (typeof pageSize === 'number') {
    tableConfig.pageSize = pageSize;
    hasValue = true;
  }

  return hasValue ? tableConfig : undefined;
}

function mapToThresholds(value: unknown): Threshold[] | undefined {
  ...
  value.forEach((item) => {
    ...
    if (
      ...
      color &&
      condition &&
      ['gt', 'gte', 'lt', 'lte', 'eq'].includes(condition)
    ) {
      thresholds.push({
        value: parsedValue,
        color,
        condition: condition as Threshold['condition'],
      });
    }
  });
...
function mapToStatConfig(config: unknown): StatConfig | undefined {
  if (!config || typeof config !== 'object') {
    return undefined;
  }

  const record = config as Record<string, unknown>;
  const valueField = typeof record.valueField === 'string'
    ? record.valueField
    : typeof record.value_field === 'string'
      ? record.value_field
      : undefined;
  ...
  const statConfig: StatConfig = { valueField };

  if (typeof record.unit === 'string') {
    statConfig.unit = record.unit;
  }

  if (typeof record.decimals === 'number') {
    statConfig.decimals = record.decimals;
  }

  const colorMode = typeof record.colorMode === 'string'
    ? record.colorMode
    : typeof record.color_mode === 'string'
      ? record.color_mode
      : undefined;
  if (colorMode === 'value' || colorMode === 'background') {
    statConfig.colorMode = colorMode;
  }

  const thresholds = mapToThresholds(record.thresholds);
  if (thresholds) {
    statConfig.thresholds = thresholds;
  }

  return statConfig;
}

function mapToGaugeConfig(config: unknown): GaugeConfig | undefined {
  if (!config || typeof config !== 'object') {
    return undefined;
  }
  ...
  if (typeof record.unit === 'string') {
    gaugeConfig.unit = record.unit;
  }

  const thresholds = mapToThresholds(record.thresholds);
  if (thresholds) {
    gaugeConfig.thresholds = thresholds;
  }

  return gaugeConfig;
}

function mapToDrilldownConfig(config: unknown): DrilldownConfig | undefined {
  if (!config || typeof config !== 'object') {
    return undefined;
  }
  ...
  const enabled = typeof enabledValue === 'boolean' ? enabledValue : Boolean(enabledValue);
  ...
  const targetPanelId = record.targetPanelId ?? record.target_panel_id;
  if (typeof targetPanelId === 'string') {
    drilldown.targetPanelId = targetPanelId;
  }

  const targetDashboardId = record.targetDashboardId ?? record.target_dashboard_id;
  if (typeof targetDashboardId === 'string') {
    drilldown.targetDashboardId = targetDashboardId;
  }

  if (record.parameterMapping && typeof record.parameterMapping === 'object') {
    const mappingEntries = Object.entries(record.parameterMapping as Record<string, unknown>)
      .filter(([, value]) => typeof value === 'string');
    if (mappingEntries.length > 0) {
      drilldown.parameterMapping = Object.fromEntries(mappingEntries) as Record<string, string>;
    }
  }
...
function mapApiPanelToDomain(panel: ApiPanelConfig): PanelConfig {
  ...
  return {
    ...
    visualization: {
      ...
      echartsOptions: mapToEChartsOptions(visualization.echarts_option),
      tableConfig: mapToTableConfig(visualization.table_config),
      statConfig: mapToStatConfig(visualization.stat_config),
      gaugeConfig: mapToGaugeConfig(visualization.gauge_config),
    },

    variables: panel.variables,

    drilldown: mapToDrilldownConfig(panel.drilldown_config),
...
export class DashboardManager extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      throw new DashboardError(
        `Failed to initialize DashboardManager: ${err.message}`,
        'MANAGER_INIT_FAILED',
        undefined,
        undefined,
        { originalError: err }
  ...
  async loadDashboard(id: string): Promise<Dashboard> {
    ...
    try {
      ...
      throw new DashboardError(
        `Failed to load dashboard ${id}: ${err.message}`,
  ...
  async createDashboard(config: DashboardConfig): Promise<Dashboard> {
    ...
    try {
      ...
      throw new DashboardError(
        `Failed to create dashboard: ${err.message}`,
  ...
  async updateDashboard(id: string, updates: Partial<DashboardConfig>): Promise<Dashboard> {
    ...
    try {
      ...
      throw new DashboardError(
        `Failed to update dashboard ${id}: ${err.message}`,
  ...
  async deleteDashboard(id: string): Promise<boolean> {
    ...
    try {
      ...
      throw new DashboardError(
        `Failed to delete dashboard ${id}: ${err.message}`,
  ...
  async loadAllDashboards(): Promise<LoadResult[]> {
    ...
    const settledResults = await Promise.allSettled(
      dashboards.map(async (dashboard) => {
        ...
        try {
          ...
          const err = error instanceof Error ? error : new Error(String(error));
          return [{
            panelId: `${dashboard.id}::dashboard`,
            success: false,
            error: err,
    ...
    settledResults.forEach((result, index) => {
      if (result.status === 'fulfilled') {
        ...
        aggregated.push({
          panelId: `${dashboard?.id ?? 'unknown'}::dashboard`,
          success: false,
          error: err,
          executionTime: 0,
        });
