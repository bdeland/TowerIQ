...
pub fn run() {
    tauri::Builder::default()
        ...
        .setup(|_app| {
            // Only start the backend sidecar in release mode
            // In dev mode, start.py handles starting the backend from source
            ...
            {
                println!("🔧 Dev mode: Backend should be started by start.py");
            }
